...
class AzureBlobManager:
    def __init__(self):
        ...
        if self.connection_string and "DefaultEndpointsProtocol" in self.connection_string:
            try:
                ...
                print(f"Azure Blob Connection Failed: {str(e)}")
    ...
    def sync_asset_from_cloud(self, blob_name: str, local_target_path: str):
        ...
        # QUICK LOCAL BYPASS: If file present in local bypass the cloud download
        ...
        if not self.connection_string:
            print(f"Storage connection not bound. Using local cached asset: {local_target_path}")
            return local_target_path
        ...
        try:
            ...
            print(f"Cloud download failed for {blob_name}: {str(err)}. Using local fallback.")
            return local_target_path