...
def load_and_chunk_txt_file(file_path: str) -> list:
    ...
    # Splitting sections or heavy paragraphs
    ...
    for idx, chunk in enumerate(raw_chunks[1:], start=1):
        ...
        # Identify category using string matching to assign the correct partition key.
        category = "general"
...
def seed_cosmos_vector_store():
    ...
    for idx, doc in enumerate(documents_corpus):
        ...
        print(f" Ingested text array: {doc['id']} [{doc['category']}] -> Cloud Storage.")

    print("\nDynamic Cosmos DB Vector Store setup completely finalized out of the text file!")
